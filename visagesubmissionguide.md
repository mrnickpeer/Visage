...
* **Summary Description (<=132 chars):**  
  `Digital Footprint & Identity Privacy Audit Workstation. Audit public exposure, check leaked credentials, and evaluate privacy risks.`
* **Detailed Description:**  
  ```text
  Visage is an open-source, client-side digital footprint and identity privacy audit workstation designed for privacy professionals, security analysts, and individuals to evaluate their public exposure and reduce social engineering risks.

  Key Features:
  • 100+ Platform Presence Matrix: Passive presence verification across developer, social, professional, gaming, and public platforms with rate-limit protection.
  • Handle Permutation Engine: Generate standardized handle conventions to audit digital shadows and forgotten legacy accounts.
  • OpenPGP Keyserver & Keybase Proofs: Discover verified email signatures, linked services, and public identity proofs.
  • Credential Compromise & Leak Telemetry: Audit email exposure against breach indexes with private in-browser k-Anonymity SHA-1 queries.
  • Multi-Tier Public Records Directory: County assessors, state corporate/court registries, and federal archives.
  • Exposure Findings & CSV/Markdown Export: Categorize exposure leads with risk severity ratings and export audit reports cleanly to CSV and Markdown.
...
> *Personal identity privacy auditing, cross-platform username presence verification, leaked credential exposure checking, cryptographic key correlation, and public records directory triage for defensive security research.*

#### B. Permission Justifications (Copy-Paste Ready)
* **`storage`**:  
  *Used to locally persist audited profile scopes, discovered platform accounts, exposure findings, remediation notes, and saved identity profiles. No data is transmitted to external servers.*
* **`tabs`**:  
...
* **`contextMenus`**:  
  *Adds right-click options ("Audit in Visage", "Save Link to Visage Exposure Log", "Save Current Page to Visage Exposure Log") so researchers can catalog discovered exposure leads directly into their local audit log.*
* **`host_permissions`**:  
...
> **Note on Downloads:** Visage does **not** request the `downloads` permission. All CSV and Markdown exports are generated client-side using standard HTML5 `Blob` and `<a download>` object URLs within the extension page context, strictly honoring the Principle of Least Privilege.
...
* **Name:** `Visage`
* **Summary (<=250 chars):**  
  `Digital Footprint & Identity Privacy Audit Workstation. Zero-touch, client-side personal exposure audit tool to evaluate privacy risks and identify leaked credentials.`
* **Description:** Paste the detailed description from Section 1.
* **Categories:** `Security & Privacy`, `Web Development`
* **Tags:** `privacy`, `security`, `audit`, `identity`, `footprint`
